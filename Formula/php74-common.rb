...
class Php74Common < Formula
  ...
  def config_path_php_fpm_www
      etc / "php" / "7.4" / "php-fpm.d" / "www.conf"
  end
  ...
  def supervisor_config_path
      supervisor_config_dir / "php74-fpm.ini"
  end
  ...
  def nginx_config_path
      nginx_config_dir / "php74.conf"
  end

  def nginx_snippet_file
     <<~EOS
        if (-f $documentRoot/.php74) {
          set $php_version 74;
        }
     EOS
  ...
  def config_file
      <<~EOS
        [program:php74]
        command=#{HOMEBREW_PREFIX}/opt/php74/sbin/php-fpm --nodaemonize --fpm-config #{HOMEBREW_PREFIX}/etc/php/7.4/php-fpm.conf
        directory=#{HOMEBREW_PREFIX}/opt/php74
        stdout_logfile=#{HOMEBREW_PREFIX}/var/log/php74-supervisor.log
        stdout_logfile_maxbytes=1MB
        stderr_logfile=#{HOMEBREW_PREFIX}/var/log/php74-supervisor.err
  ...
  def install
    ...
    begin
        inreplace config_path_php_fpm_www do |s|
            ...
            s.sub!(/^.*?listen\s*=.+$/, "listen = 127.0.0.1:9074")
        end
    rescue StandardError
        nil
    end
