...
class Php73Ioncubeloader < AbstractPhp73Extension
  ...
  def install
    if OS.mac?
      prefix.install "ioncube_loader_dar_7.3.so" => "ioncubeloader.so" if build.without? "thread-safe"
      prefix.install "ioncube_loader_dar_7.3_ts.so" => "ioncubeloader_ts.so" if build.with? "thread-safe"
    elsif OS.linux?
