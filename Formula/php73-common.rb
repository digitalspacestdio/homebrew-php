...
class Php73Common < Formula
  ...
  def supervisor_config_path
      supervisor_config_dir / "php73-fpm.ini"
  end
  ...
  def nginx_config_path
      nginx_config_dir / "php73.conf"
  end

  def nginx_snippet_file
     <<~EOS
        if (-f $documentRoot/.php73) {
          set $php_version 73;
        }
     EOS
  ...
  def config_file
      <<~EOS
        [program:php73]
        command=#{HOMEBREW_PREFIX}/opt/php73/sbin/php-fpm --nodaemonize --fpm-config #{HOMEBREW_PREFIX}/etc/php/7.3/php-fpm.conf
        directory=#{HOMEBREW_PREFIX}/opt/php73
        stdout_logfile=#{HOMEBREW_PREFIX}/var/log/php73-supervisor.log
        stdout_logfile_maxbytes=1MB
        stderr_logfile=#{HOMEBREW_PREFIX}/var/log/php73-supervisor.err
  ...
  def install
    ...
    begin
        inreplace config_path_php_fpm_www do |s|
            ...
            s.sub!(/^.*?listen\s*=.+$/, "listen = 127.0.0.1:9073")
        end
    rescue StandardError
        nil
    end
