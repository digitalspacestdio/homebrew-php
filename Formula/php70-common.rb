...
class Php70Common < Formula
  ...
  def supervisor_config_path
      supervisor_config_dir / "php70-fpm.ini"
  end
  ...
  def config_file
      <<~EOS
        [program:php70]
        command=#{HOMEBREW_PREFIX}/opt/php70/sbin/php-fpm --nodaemonize --fpm-config #{HOMEBREW_PREFIX}/etc/php/7.0/php-fpm.conf
        directory=#{HOMEBREW_PREFIX}/opt/php70
        stdout_logfile=#{HOMEBREW_PREFIX}/var/log/php70-supervisor.log
        stdout_logfile_maxbytes=1MB
        stderr_logfile=#{HOMEBREW_PREFIX}/var/log/php70-supervisor.err
  ...
  def install
    ...
    begin
        inreplace config_path_php_fpm_www do |s|
            ...
            s.sub!(/^.*?listen\s*=.+$/, "listen = 127.0.0.1:9070")
        end
    rescue StandardError
        nil
    end
